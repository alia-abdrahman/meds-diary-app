...
struct SupplementFormView: View {
    ...
    init(supplement: Supplement? = nil) {
        ...
        let nameVal = supplement?.name ?? ""
        let dosageVal = supplement?.dosage ?? ""
        let frequencyVal = supplement?.frequency ?? "Daily"
        let reminderVal = supplement?.reminderTimes ?? []
        let notesVal = supplement?.notes ?? ""
        let activeVal = supplement?.isActive ?? true
        let stockValue = supplement?.stock ?? 0
        let stockVal = stockValue > 0 ? "\(stockValue)" : ""
        let imageVal = supplement?.imageData
    ...
    var body: some View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    HStack {
                        ...
                        Text(isEditing ? "Edit Supplement" : "New Supplement")
                    ...
                    VStack(spacing: 0) {
                        // Name
                        formRow(icon: "info.circle.fill", iconColor: PastelTheme.dark, iconBg: PastelTheme.primary.opacity(0.2), label: "Name") {
                            TextField("Enter supplement name", text: $name)
                        ...
                        formRow(icon: "cross.case.fill", iconColor: PastelTheme.dark, iconBg: PastelTheme.primary.opacity(0.2), label: "Dosage") {
                            TextField("e.g. 1000mg", text: $dosage)
                                .font(.poppins(.regular, size: 15))
                                .multilineTextAlignment(.trailing)
                        }
                        ...
                        // Stock
                        formRow(icon: "shippingbox.fill", iconColor: PastelTheme.dark, iconBg: PastelTheme.primary.opacity(0.2), label: "Stock") {
                            TextField("Number of pills", text: $stock)
                                .font(.poppins(.regular, size: 15))
                    ...
                    TextField("Add any notes about this supplement", text: $notes, axis: .vertical)
                        .font(.poppins(.regular, size: 15))
                        .lineLimit(3...6)
                        .padding(14)
                        .background(.white)
        ...
        .fullScreenCover(isPresented: $showCamera) {
            SupplementCameraPicker { image in
...
private struct SupplementCameraPicker: UIViewControllerRepresentable {
    @Environment(\.dismiss) private var dismiss
    ...
    class Coordinator: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        let parent: SupplementCameraPicker
        init(_ parent: SupplementCameraPicker) { self.parent = parent }
